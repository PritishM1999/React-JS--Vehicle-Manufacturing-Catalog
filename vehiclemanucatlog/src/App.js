...
function App() {
  ...
  useEffect(() => {
    setFilteredManufacturers(manufacturers.filter(result =>
      ...
      (result.Country && result.Country.toLowerCase().includes(searchTerm.toLowerCase())) ||
      (result.VehicleTypeName && result.VehicleTypeName.toLowerCase().includes(searchTerm.toLowerCase()))
    ));
